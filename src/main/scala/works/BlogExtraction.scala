...
class BlobExtractionDemo {
  ...
  def run() {
    ...
    val image = Imgcodecs.imread("./src/main/resources/svg2.png")
    ...
    val clusters = cluster(image, 3).get(0)

    val filename = "bloboutput.png"
    println(String.format("Writing %s", filename))
    Imgcodecs.imwrite(filename, clusters)
